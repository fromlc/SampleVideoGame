# Sample Video Game for CS 281

This code is derived from Dr. Churchill's COMP 4300 Assignment 2,
presented on YouTube: https://youtu.be/-ugbLQlw_VM?si=tZnH4CMx3jueOFG5

Dr. C's Entity Component System architecture for A2 is simple and
well-suited for intermediate C++ students. He includes instructive
features of C++, OOP, and ECS. He uses SFML v2.5.1, an easy
graphics library to produce 2D games with.

Dr. C only provides assignment starter code to enrolled students,
but his public video lectures are detailed enough to make A2 doable
from scratch, as here. 

This game is similar to A2 but simpler and it uses SFML v3.0.2.
This code retains Dr. C's ECS architecture and instructive 
features, but these modifications make it useless as A2 solution
code.

## SFML

This Visual Studio 2022 solution links with downloaded 64-bit
SFML 3.0.2 static libraries for Windows.

https://www.sfml-dev.org/download/sfml/3.0.2/#windows

https://www.sfml-dev.org/tutorials/3.0/getting-started/visual-studio/

https://www.sfml-dev.org/tutorials/3.0/getting-started/visual-studio/#creating-and-configuring-an-sfml-project

SFML License: https://www.sfml-dev.org/license/

## Dear ImGui

Dear Imgui v1.91.9-d source code compiles with solution source 
code.

https://github.com/ocornut/imgui/tree/v1.91.9-docking

https://github.com/ocornut/imgui/wiki/Getting-Started

https://pthom.github.io/imgui_manual_online/

https://www.dearimgui.com/

Dear Imgui License: https://www.dearimgui.com/licenses/

## ImGui-SFML

Dear ImGui backend for SFML. ImGui-SFML v3.0 source code
also compiles with solution source code.

https://github.com/SFML/imgui-sfml/tree/v3.0

## CMake Tutorials

This solution supports 64-bit Windows. A CMake version for
Linux and Mac development is in the works.

https://edw.is/using-cmake/

https://www.sfml-dev.org/tutorials/3.0/getting-started/cmake/#introduction

https://github.com/SFML/cmake-sfml-project

## References

Dr. Churchill at Memorial University and on YouTube

https://www.mun.ca/

https://www.youtube.com/@DaveChurchill

Dr. Churchill's Lectures on Assignment 2

https://youtu.be/DcMiPoNOmHg?si=9J7h3GxDm7KF6II-

https://youtu.be/cK_LbRWaIOU?si=2QQzvpcWFJL6zJdB

Austin Morlan's Simple ECS Implementation (2019)

https://code.austinmorlan.com/austin/2019-ecs
